...
const ProfilePage: NextPage<InferGetStaticPropsType<typeof getStaticProps>> = ({ id }) => {
    ...
    return <>
        <Head>
            <title>{`wAve - ${profile.name}`}</title>
        </Head>
        <header className="sticky top-0 z-10 flex items-center border-b bg-transparent px-4 py-2">
...
function FollowButton({ userId, isFollowing, isLoading, onClick }: { userId: string, isFollowing: boolean, isLoading: boolean, onClick: () => void } ) {
    ...
    return <Button disabled={isLoading} onClick={onClick} small gray={isFollowing} className="rounded-full text-lg mr-5">
