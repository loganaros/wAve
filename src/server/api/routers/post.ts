...
async function getInfinitePosts({ whereClause, ctx, limit, cursor }: { whereClause?: Prisma.PostWhereInput, limit: number, cursor: { id: string, createdAt: Date} | undefined, ctx: inferAsyncReturnType<typeof createTRPCContext>;}) {
  const currentUserId = ctx.session?.user.id
